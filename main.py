...
class LocalOwl:
    """Reviews GitHub PRs via polling (default) or webhooks (set WEBHOOK_SECRET)."""
    ...
    def handle_webhook_pr(self, repo: str, pr_number: int, head_sha: str, is_draft: bool):
        """Called by the webhook server for each incoming PR event."""
        if SKIP_DRAFT_PRS and is_draft:
            log.info("[%s] Skipping draft PR #%d (webhook)", repo, pr_number)
            return

        # Deduplicate — don't re-review the same commit twice
